...
-- Script de données de test pour les tables adherent et utilisateur

-- 1. Insérer des types d'utilisateur (si pas déjà fait)
...
VALUES 
('ETUDIANT', 'Étudiant', 15, 5, 2, 1, true, true),
...
-- Professeurs
('Dubois', 'Jean', 'jean.dubois@email.com', (SELECT id_type_utilisateur FROM type_utilisateur WHERE code_type = 'PROFESSEUR'), '2023-09-01', '2025-09-01', 'ACTIF', '{"email": true, "sms": true}', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('Moreau', 'Claire', 'claire.moreau@email.com', (SELECT id_type_utilisateur FROM type_utilisateur WHERE code_type = 'PROFESSEUR'), '2023-10-15', '2025-10-15', 'ACTIF', '{"email": true, "sms": false}', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('Leroy', 'Michel', 'michel.leroy@email.com', (SELECT id_type_utilisateur FROM type_utilisateur WHERE code_type = 'PROFESSEUR'), '2023-11-20', '2025-11-20', 'ACTIF', '{"email": true, "sms": true}', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),

-- Personnel
('Roux', 'Anne', 'anne.roux@email.com', (SELECT id_type_utilisateur FROM type_utilisateur WHERE code_type = 'PERSONNEL'), '2023-08-01', '2025-08-01', 'ACTIF', '{"email": true, "sms": true}', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('Simon', 'Paul', 'paul.simon@email.com', (SELECT id_type_utilisateur FROM type_utilisateur WHERE code_type = 'PERSONNEL'), '2023-12-01', '2025-12-01', 'ACTIF', '{"email": true, "sms": false}', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),

-- Utilisateurs externes
('Michel', 'Isabelle', 'isabelle.michel@email.com', (SELECT id_type_utilisateur FROM type_utilisateur WHERE code_type = 'EXTERNE'), '2024-04-01', '2025-04-01', 'ACTIF', '{"email": true, "sms": false}', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('Garcia', 'Carlos', 'carlos.garcia@email.com', (SELECT id_type_utilisateur FROM type_utilisateur WHERE code_type = 'EXTERNE'), '2024-05-15', '2025-05-15', 'ACTIF', '{"email": false, "sms": true}', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);

-- 3. Insérer des utilisateurs (comptes de connexion)
...
VALUES 
-- Utilisateurs avec adhérents (étudiants)
...
-- Utilisateurs avec adhérents (professeurs)
...
-- Utilisateurs avec adhérents (personnel)
...
-- Utilisateurs avec adhérents (externes)
...
-- Utilisateurs système (bibliothécaires et administrateurs)
...
('bibliothecaire1', 'biblio123', 'biblio1@bibliotheque.com', 'Bibliothécaire Principal', 'BIBLIOTHECAIRE', (SELECT id_type_utilisateur FROM type_utilisateur WHERE code_type = 'PERSONNEL'), NULL, true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('bibliothecaire2', 'biblio123', 'biblio2@bibliotheque.com', 'Bibliothécaire Adjoint', 'BIBLIOTHECAIRE', (SELECT id_type_utilisateur FROM type_utilisateur WHERE code_type = 'PERSONNEL'), NULL, true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('responsable', 'resp123', 'responsable@bibliotheque.com', 'Responsable Bibliothèque', 'RESPONSABLE', (SELECT id_type_utilisateur FROM type_utilisateur WHERE code_type = 'PERSONNEL'), NULL, true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);

-- 4. Afficher les données insérées (pour vérification)
SELECT 'Types d\'utilisateur:' as info;
SELECT * FROM type_utilisateur;

SELECT 'Adhérents:' as info;
SELECT a.id_adherent, a.nom, a.prenom, a.email, t.nom_type, a.statut, a.actif 
FROM adherent a 
